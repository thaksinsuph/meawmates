...
const fixImage = (img) => {
    ...
    if (img.startsWith("blob:")) return img; // ถ้าเป็น Blob URL (Preview)
    if (img.startsWith("http")) return img;  // ถ้าเป็น URL สมบูรณ์จาก Cloudinary
...
const GenderSelect = ({ value, onChange }) => {
    const genders = [
        { value: "Male", label: "Male", img: "/images/male.png", style: "border-blue-300 text-blue-500 hover:bg-blue-50" },
        { value: "Female", label: "Female", img: "/images/female.png", style: "border-pink-300 text-pink-500 hover:bg-pink-50" },
    ];
        ...
        <div className="flex gap-3 mb-4">
            {genders.map((g) => (
                <button
                    ...
                    className={`flex items-center justify-center gap-2 p-3 flex-1 rounded-xl font-medium transition-all shadow-sm border-2 ${g.style} ${value === g.value ? 'ring-2 ring-offset-2 ring-pink-500' : 'bg-white/80 border-gray-200 text-gray-700'}`}
                >
...
export default function ManagePet() {
    ...
    const [popup, setPopup] = useState({ open: false, title: "", img: "", desc: "" });
    ...
    const handlePetdreegreeImage = (e) => {
        ...
        if (file) {
            ...
            setPetdreegeePreview(URL.createObjectURL(file));
        }
    };
    ...
    const savePet = async () => {
        if (!form.name || !form.breed || !form.color || !form.age || !form.gender || !form.province) 
        ...
        try {
            ...
            if (hasPetdreegree) {
                ...
                } else if (form.PetdreegreeImage) {
                    formData.append("PetdreegreeImage", form.PetdreegreeImage);
                }
            } else {
    ...
    return (
        <div className="w-full flex flex-col items-center py-12 px-4 gap-12 bg-gray-50 min-h-screen">
            ...
            <h1 className="text-4xl font-bold flex items-center gap-3 text-gray-800 text-center">
                ...
                {allPets.map((pet, i) => (
                    <div key={i} className={`p-4 rounded-2xl shadow-md cursor-pointer border-2 bg-white transition-all ${selectedSlot === i + 1 ? "border-pink-500 ring-4 ring-pink-100 scale-105" : "border-gray-100 hover:border-pink-200"}`} onClick={() => setSelectedSlot(i + 1)}>
                        <div className="w-full h-32 bg-gray-50 rounded-xl overflow-hidden flex items-center justify-center border shadow-inner">
                            {pet?.image ? <img src={fixImage(pet.image)} className="w-full h-full object-cover" alt="cat" /> : <span className="text-gray-300 font-bold">Slot Empty</span>}
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div className="flex flex-col items-center bg-gray-50 rounded-[2.5rem] p-8 border-2 border-dashed border-gray-200">
                        ...
                        <div className="w-72 h-72 bg-white border-4 border-white rounded-[3rem] shadow-2xl overflow-hidden flex items-center justify-center relative group">
                            {preview ? <img src={preview} className="w-full h-full object-cover" alt="preview" /> : <div className="text-gray-300 text-center font-bold">No picture<br/>Click to upload</div>}
                        ...
                        <div className="mt-10 w-full flex flex-col items-center pt-8 border-t border-gray-200">
                            <p className="font-black text-gray-700 mb-5 text-center lowercase">DO YOU HAVE A PETDREEGREE RECORD?</p>
                            <div className="flex gap-4 mb-8">
                                <button type="button" onClick={() => setHasPetdreegree(true)} className={`px-10 py-3 rounded-2xl font-black border-2 transition-all ${hasPetdreegree ? 'bg-blue-500 text-white border-blue-500 shadow-lg shadow-blue-100' : 'bg-white text-gray-300 border-gray-200'}`}> YES </button>
                                <button type="button" onClick={() => { setHasPetdreegree(false); setPetdreegeePreview(null); setForm({...form, PetdreegreeImage: null, PetdreegreeImageFile: null}); }} className={`px-10 py-3 rounded-2xl font-black border-2 transition-all ${!hasPetdreegree ? 'bg-gray-400 text-white border-gray-400 shadow-lg shadow-gray-100' : 'bg-white text-gray-300 border-gray-200'}`}> NO </button>
                            </div>

                            {hasPetdreegree && (
                                <div className="w-full flex flex-col items-center">
                                    <div className="w-full h-44 bg-white border-2 border-dashed border-blue-200 rounded-[2rem] overflow-hidden shadow-inner flex items-center justify-center relative">
                                        {PetdreegeePreview ? <img src={PetdreegeePreview} className="w-full h-full object-contain p-2" alt="pedigree" /> : <div className="text-blue-300 text-xs text-center font-black">CLICK TO UPLOAD<br/>PETDREEGREE RECORD</div>}
                                        <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={handlePetdreegreeImage} accept="image/*" />
                                    </div>
                                    {PetdreegeePreview && (
                                        <button className="mt-4 text-red-500 underline text-xs font-black" onClick={() => {setPetdreegeePreview(null); setForm({...form, PetdreegreeImage: null, PetdreegreeImageFile: null});}}> Remove Record </button>
                                    )}
                                </div>
                            )}
                        </div>
